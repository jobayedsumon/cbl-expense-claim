...
function approve_claim() {
    if ($("input.chk_claim:checked").length > 1) {
        bootbox.confirm({
            message: "Are you sure you want to approve batch claims?",
            buttons: {
                confirm: {
                    label: 'Yes',
                ...
                if (shouldApprove) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            bootbox.alert({
                                message: 'All claims have been approved successfully.',
        ...
        bootbox.confirm({
            message: "Are you sure you want to approve the claim?",
            buttons: {
                confirm: {
                    label: 'Yes',
                    className: 'btn-success'
            ...
            callback: function (shouldApprove) {
                if (shouldApprove) {
                    selected_tr = $("table#my_table tr.selected");
                    var id = selected_tr.find("div.exc_claim_requests").eq(0).attr('exc_claim_requests_id');
                    var data = JSON.stringify({
                        "CURRENT_APPROVAL_PERSON": $('#currentApprovalPerson').val(),
                        "EXC_CLAIM_REQUESTS_ID": id,
                        "BUTTON": "APPROVE"
                    });
                    $.ajax({
                        url: EXCS_URL+'/excs/claims/process',
